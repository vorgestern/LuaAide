...
static int mynew(lua_State*L)
{
    ...
    if (Q.hastableat(-2))
    {
        const int a=lua_geti(L, -2, 1)==LUA_TNUMBER?lua_tointeger(L, -1):101; lua_pop(L, 1);
        const int b=lua_geti(L, -2, 2)==LUA_TNUMBER?lua_tointeger(L, -1):102; lua_pop(L, 1);
        const int c=lua_geti(L, -2, 3)==LUA_TNUMBER?lua_tointeger(L, -1):103; lua_pop(L, 1);
